...
data class ConfigBean(
    ...
    fun displayOpenAdWithInsertAd() = i == 1

    fun isCanDisplayInsertAd() = needLogin()

    fun isCanDisplayByPercent() = Random.nextInt(1, 101) <= lr?.toIntOrNull() ?: 0

    fun insertAdInvokeTime() = ext1?.split(",")?.getOrNull(0)?.toIntOrNull() ?: -1

    fun insertAdRealTime() = ext1?.split(",")?.getOrNull(1)?.toIntOrNull() ?: -1

    fun insertAdInterval() = ext1?.split(",")?.getOrNull(2)?.toIntOrNull() ?: -1
